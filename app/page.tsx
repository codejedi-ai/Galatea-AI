...
export default function Home() {
  ...
  useEffect(() => {
    const check = async () => {
      ...
      if (!url || !key) { setSupabaseConnected(false); return }
      try {
        const controller = new AbortController()
        const timeout = setTimeout(() => controller.abort(), 3000)
        const res = await fetch(`${url}/rest/v1/agents?limit=1`, {
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      {supabaseConnected === false && (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-teal-500/10 border-t border-teal-500/20 px-4 py-2 flex items-center justify-center gap-2 text-teal-400 text-sm font-mono">
