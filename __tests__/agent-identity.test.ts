...
async function generateAgentId(name: string, publicKey: string, framework: string): Promise<string> {
...
async function buildAttestationSignature(agentId: string, timestamp: string, nonce: string, publicKey: string): Promise<string> {
...
describe("FrameworkSchema", () => {
  it("accepts valid frameworks", () => {
    for (const framework of ["LangChain", "AutoGen", "CrewAI", "Custom"] as const) {
...
describe("shortAgentId", () => {
  ...
  it("handles non-standard prefixes gracefully", () => {
